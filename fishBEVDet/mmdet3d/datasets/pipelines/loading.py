...
class LoadPointsFromFile(object):
    ...
    def __repr__(self):
        ...
        return repr_str
@PIPELINES.register_module()
class LoadPointsFromFile4D(object):
    """Load 4D Points From File.

    Load 4D point clouds from file (x, y, z, intensity/reflectance).
    ...
    Args:
        coord_type (str): The type of coordinates of points cloud.
            ...
            - 'CAMERA': Points in camera coordinates.
        load_dim (int, optional): The dimension of the loaded points.
            Defaults to 4 for (x, y, z, intensity).
        use_dim (list[int], optional): Which dimensions of the points to use.
            Defaults to [0, 1, 2, 3] to use all 4 dimensions.
        shift_height (bool, optional): Whether to use shifted height.
            Defaults to False.
        intensity_dim (int, optional): Which dimension represents intensity.
            Defaults to 3.
        normalize_intensity (bool, optional): Whether to normalize intensity values.
            Defaults to False.
        intensity_range (tuple, optional): Range for intensity normalization.
            Defaults to None.
        file_client_args (dict, optional): Config dict of file clients,
            refer to
            https://github.com/open-mmlab/mmcv/blob/master/mmcv/fileio/file_client.py
    ...
    def __init__(self,
        ...
        assert max(use_dim) < load_dim, \
            f'Expect all used dimensions < {load_dim}, got {use_dim}'
        assert coord_type in ['CAMERA', 'LIDAR', 'DEPTH']
        assert load_dim >= 4, f'Expected load_dim >= 4 for 4D points, got {load_dim}'
    ...
    def _process_intensity(self, points):
        """Process intensity values (normalization, clipping, etc.).

        Args:
            points (np.ndarray): Point cloud data.

        Returns:
            np.ndarray: Points with processed intensity.
        """
        if self.normalize_intensity and points.shape[1] > self.intensity_dim:
            intensity = points[:, self.intensity_dim]
            
            if self.intensity_range is not None:
                # Normalize to specified range
                min_val, max_val = self.intensity_range
                intensity = np.clip(intensity, min_val, max_val)
            ...
            points[:, self.intensity_dim] = intensity
        
        return points

    def __call__(self, results):
        """Call function to load 4D points data from file.

        Args:
            results (dict): Result dict containing point clouds data.

        Returns:
            dict: The result dict containing the 4D point clouds data.
                Added key and value are described below.

                - points (:obj:`BasePoints`): 4D Point clouds data.
        """
        pts_filename = results['pts_filename']
        points = self._load_points(pts_filename)
        points = points.reshape(-1, self.load_dim)
        
        # Process intensity if needed
        points = self._process_intensity(points)
        ...
        # Set up attribute dimensions for 4D points
        if points.shape[1] >= 4:
            if attribute_dims is None:
                attribute_dims = dict()
            
            # Add intensity attribute
            if self.intensity_dim in self.use_dim:
                intensity_idx = self.use_dim.index(self.intensity_dim)
                attribute_dims.update(dict(intensity=intensity_idx))
    ...
    def __repr__(self):
        ...
        repr_str += f'use_dim={self.use_dim}, '
        repr_str += f'shift_height={self.shift_height}, '
        repr_str += f'intensity_dim={self.intensity_dim}, '
        repr_str += f'normalize_intensity={self.normalize_intensity}, '
        repr_str += f'intensity_range={self.intensity_range}, '
        repr_str += f'file_client_args={self.file_client_args})'
        return repr_str
...
class PrepareImageInputs(object):
    ...
    def get_inputs(self, results, flip=None, scale=None):
        ...
        canvas = []
        target_height, target_width = self.data_config['src_size']
        fisheye_crop_regions = self.data_config.get('fisheye_crop_regions', {})
        # print(f"[PrepareImageInputs] Using fisheye_crop_regions: {fisheye_crop_regions}")
        enable_viz = self.data_config.get('enable_visualization', False)
        save_dir = self.data_config.get('save_dir', './fisheye_debug_images')
        max_viz_count = self.data_config.get('max_viz_count', 10)
        if enable_viz:
            if not hasattr(self, '_viz_counter'):
                self._viz_counter = 0
            os.makedirs(save_dir, exist_ok=True)
        fisheye_cams = ['CAM_BACK_LEFT', 'CAM_BACK_RIGHT']
        for cam_name in cam_names:
            ...
            # Visualization: original image (before crop/resize)
            orig_img_np = np.array(img)

            intrin = torch.Tensor(cam_data['cam_intrinsic'])
            if cam_data['dist_coeffs'] is not None:
                dist_coeff = torch.Tensor(cam_data['dist_coeffs'])
            else:
                dist_coeff = torch.zeros(5)
            # Handle fisheye camera cropping
            cropped_img_np = None
            if cam_name in fisheye_crop_regions:
                crop_region = fisheye_crop_regions[cam_name]
                img = img.crop(crop_region)
                # Adjust intrinsic matrix for the crop
                intrin[0, 2] -= crop_region[0]  # cx
                intrin[1, 2] -= crop_region[1]  # cy
                # scale the img and intrinsic to the target size
                if img.height != target_height or img.width != target_width:
                    intrin[0, :] *= target_width / img.width
                    intrin[1, :] *= target_height / img.height
                    img = img.resize((target_width, target_height))
                # Visualization: cropped and resized image
                cropped_img_np = np.array(img)
            else:
                # If not fisheye, just use the resized image for visualization
                if img.height != target_height or img.width != target_width:
                    img = img.resize((target_width, target_height))
                cropped_img_np = np.array(img)
            ...
            # Visualization: after img_transform
            img_transform_np = np.array(img)

            # Only visualize for fisheye cams, and only if enabled and under max count
            if enable_viz and cam_name in fisheye_cams and self._viz_counter < max_viz_count:
                fig, axs = plt.subplots(1, 3, figsize=(15, 5))
                axs[0].imshow(orig_img_np)
                axs[0].set_title(f'{cam_name} original')
                axs[0].axis('off')
                axs[1].imshow(cropped_img_np)
                axs[1].set_title(f'{cam_name} cropped/resized')
                axs[1].axis('off')
                axs[2].imshow(img_transform_np)
                axs[2].set_title(f'{cam_name} after img_transform')
                axs[2].axis('off')
                plt.tight_layout()
                save_path = os.path.join(save_dir, f'{cam_name}_viz_{self._viz_counter+1:03d}.png')
                plt.savefig(save_path)
                plt.close(fig)
                self._viz_counter += 1
                if self._viz_counter >= max_viz_count:
                    print(f"[Fisheye Visualization] Reached max_viz_count={max_viz_count}, stop saving more images.")
            ...
            if self.sequential:
                ...
                for adj_info in results['adjacent']:
                    ...
                    if cam_name in fisheye_crop_regions:
                        crop_region = fisheye_crop_regions[cam_name]
                        img = img_adjacent.crop(crop_region)
                        # Adjust intrinsic matrix for the crop
                        intrin[0, 2] -= crop_region[0]  # cx
                        intrin[1, 2] -= crop_region[1]  # cy
                        # scale the img and intrinsic to the target size
        ...
        if self.sequential:
            for adj_info in results['adjacent']:
                ...
                dist_coeffs.extend(dist_coeffs[:len(cam_names)])
        ...
        dist_coeffs = torch.stack(dist_coeffs)
        post_rots = torch.stack(post_rots)
        post_trans = torch.stack(post_trans)
        results['canvas'] = canvas
        return (imgs, sensor2egos, ego2globals, intrins, dist_coeffs, post_rots, post_trans)
...
class BEVAug(object):
    ...
    def __call__(self, results):
        ...
        if 'img_inputs' in results:
            imgs, rots, trans, intrins, dist_coeffs = results['img_inputs'][:5]
            post_rots, post_trans = results['img_inputs'][5:]
            results['img_inputs'] = (imgs, rots, trans, intrins, post_rots,
                                     post_trans, bda_mat,dist_coeffs)
