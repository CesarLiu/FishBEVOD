...
class BEVDet(CenterPoint):
    ...
    def prepare_inputs(self, inputs):
        ...
        imgs, sensor2egos, ego2globals, intrins, post_rots, post_trans, bda, dist_coeffs= \
            inputs
        ...
        return [imgs, sensor2keyegos, ego2globals, intrins,
                post_rots, post_trans, bda, dist_coeffs]
    ...
    def simple_test(self,
        ...
        #     ]
        # for sample_idx, pts_bbox in enumerate(bbox_pts):
        #     print(f"Sample {sample_idx}:")
            
        #     if pts_bbox is not None and isinstance(pts_bbox, dict):
        #         # Check if we have the expected keys
        #         if 'boxes_3d' in pts_bbox and 'scores_3d' in pts_bbox and 'labels_3d' in pts_bbox:
        ...
        #                     count = mask.sum()
        #                     max_score = scores_np[mask].max()
        #                     mean_score = scores_np[mask].mean()
                            
        #                     cls_name = all_class_names[cls_idx] if cls_idx < len(all_class_names) else f"Unknown_{cls_idx}"
        #                     print(f"    {cls_name} (idx={cls_idx}): {count} detections, "
        #                         f"max_score={max_score:.4f}, mean_score={mean_score:.4f}")
        #             else:
        #                 print(f"    No detections above threshold")
        #         else:
        #             print(f"    Unexpected pts_bbox format: {type(pts_bbox)}")
        #             if isinstance(pts_bbox, dict):
        #                 print(f"    Available keys: {list(pts_bbox.keys())}")
        #     else:
        #         print(f"    pts_bbox is None or not a dict: {type(pts_bbox)}")
