...
#  'CAM_FRONT_LEFT'
    ...
    row = num_cams//2
    if ax is None:
        _, ax = plt.subplots(num_cams, 2, figsize=(24, 24))
    j = 0
    for ind, cam in enumerate(cams):
        ...
        if sensor_modality in ['lidar', 'radar']:
            ...
            if with_anns:
                ...
                for box in boxes_gt:
                    c = np.array(get_color(box.name)) / 255.0
                    box.render(ax[j + row, ind], view=camera_intrinsic, normalize=True, colors=(c, c, c),dist_coeffs=dist_coeffs)

            # Limit visible range.
            ax[j, ind].set_xlim(0, data.size[0])
            ax[j, ind].set_ylim(data.size[1], 0)
            ax[j + row, ind].set_xlim(0, data.size[0])
            ax[j + row, ind].set_ylim(data.size[1], 0)
        ...
        ax[j + row, ind].axis('off')
        ax[j + row, ind].set_title('GT:{} {labels_type}'.format(
            sd_record['channel'], labels_type='(predictions)' if lidarseg_preds_bin_path else ''))
        ax[j + row, ind].set_aspect('equal')
