...
def parse_args():
    ...
    parser.add_argument(
        '--canva-size', type=int, default=376, help='Size of canva in pixel')
    parser.add_argument(
        '--vis-frames',
        type=int,
        default=500,
...
def main():
    ...
    info_path = \
        args.root_path + '/bevdetv3-nuscenes-pinhole-mini_infos_val.pkl'
    ...
    if args.format == 'video':
        ...
        vout = cv2.VideoWriter(
            ...
            args.fps, (int(1408 / scale_factor * 3),
                       int(376 / scale_factor * 2 + canva_size)))
    ...
    for cnt, infos in enumerate(
        ...
        img = np.zeros((376 * 2 + canva_size * scale_factor, 1408 * 3, 3),
                       dtype=np.uint8)
        img[:376, :, :] = np.concatenate(imgs[:3], axis=1)
        img_back = np.concatenate(
            [imgs[3][:, ::-1, :], imgs[4][:, ::-1, :], imgs[5][:, ::-1, :]],
            axis=1)
        img[376 + canva_size * scale_factor:, :, :] = img_back
        img = cv2.resize(img, (int(1408 / scale_factor * 3),
                               int(376 / scale_factor * 2 + canva_size)))
        w_begin = int((1408 * 3 / scale_factor - canva_size) // 2)
        img[int(376 / scale_factor):int(376 / scale_factor) + canva_size,
