...
def add_center_dist(nusc: NuScenes,
    ...
    """
    Adds the cylindrical (xy) center distance from ego vehicle to each box.
    The box center is first transformed into the ego frame.
    """
    for sample_token in eval_boxes.sample_tokens:
        sample_rec = nusc.get('sample', sample_token)
        sd_record = nusc.get('sample_data', sample_rec['data']['LIDAR_TOP'])
        ...
        for box in eval_boxes[sample_token]:
            # Translate into ego origin, then rotate into ego frame.
            box_center_global = np.array(box.translation)
            rel_global = box_center_global - ego_translation_global
            rel_ego = ego_rot_inv.rotate(rel_global)

            if isinstance(box, DetectionBox) or isinstance(box, TrackingBox):
                box.ego_translation = tuple(rel_ego)
            else:
                raise NotImplementedError
...
def filter_eval_boxes(nusc: NuScenes,
    ...
    total, dist_filter, point_filter, bike_rack_filter, angular_filter_count = 0, 0, 0, 0, 0
    for ind, sample_token in enumerate(eval_boxes.sample_tokens):
        ...
        # eval_boxes.boxes[sample_token] = [box for box in eval_boxes[sample_token] if
        #                                   box.ego_dist < 50 and box.ego_dist >= 40]
        ...
        # angular_filter = 'all'
        # Filter by angular position
        # if angular_filter != 'all':
        #     filtered_sample_boxes = []
        #     for box in eval_boxes[sample_token]:
        #         # Calculate angle from ego vehicle to box center
        #         if hasattr(box, 'ego_translation'):
        #             # print('filtering by angle!')
        #             dx, dy, dz = box.ego_translation
                    
        #             # Calculate angle in degrees (0° = front, 90° = left, 180° = back, 270° = right)
        ...
        #             if angular_filter == 'right':
        #                 # Front: 300° to 360° and 0° to 60° (120° total)
        #                 include_box = (angle_deg >= 315) or (angle_deg <= 45)
                        
        #             elif angular_filter == 'left':
        #                 # Back: 120° to 240° (120° total)
        #                 include_box = (135 <= angle_deg <= 225)
        #             elif angular_filter == 'front':
        #                 # Sides: 60° to 120° and 240° to 300° (120° total)
        #                 include_box = (45 < angle_deg < 135) 
        #             elif angular_filter == 'back':
        #                 include_box = (225 < angle_deg < 315)
                    
        #             if include_box:
        #                 filtered_sample_boxes.append(box)
        #         else:
        #             # If ego_translation is not available, include all boxes
        #             filtered_sample_boxes.append(box)
            
        #     eval_boxes.boxes[sample_token] = filtered_sample_boxes
        
        # angular_filter_count += len(eval_boxes[sample_token])

        angular_filter = 'all'  # 'all', 'front', 'back', 'sides'
        # Filter by angular position
        if angular_filter != 'all':
            filtered_sample_boxes = []
            for box in eval_boxes[sample_token]:
                ...
                if hasattr(box, 'ego_translation'):
                    ...
                    if angular_filter == 'front':
                        # Front: 300° to 360° and 0° to 60° (120° total)
                        include_box = (angle_deg >= 300) or (angle_deg <= 60)
                    elif angular_filter == 'sides':
                        # Sides: 60° to 120° and 240° to 300° (120° total)
                        include_box = (60 < angle_deg < 120) or (240 < angle_deg < 300)
                    elif angular_filter == 'back':
                        include_box = (120 <= angle_deg <= 240)
                    
                    # if angular_filter == 'sides':
                    #     include_box = (angle_deg >= 330) or (angle_deg <= 30) or (150 <= angle_deg <= 210)
                    # elif angular_filter == 'front':
                    #     # Sides: 60° to 120° and 240° to 300° (120° total)
                    #     include_box = (30 < angle_deg < 150) 
                    # elif angular_filter == 'back':
                    #     include_box = (210 < angle_deg < 330)
    ...
    if verbose:
        ...
        print("=> After angular filtering: %d" % angular_filter_count)
