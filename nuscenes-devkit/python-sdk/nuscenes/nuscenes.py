...
class NuScenesExplorer:
    ...
    def get_color(self, category_name: str) -> Tuple[int, int, int]:
        ...
        return self.nusc.colormap.get(category_name, (150, 150, 150))
    ...
    def map_pointcloud_to_image(self,
        ...
        if cs_record['distortion_coefficients'] != "":
            dist_coeffs = np.array(cs_record['distortion_coefficients'])
        points = view_points(pc.points[:3, :], np.array(cs_record['camera_intrinsic']), normalize=True,dist_coeffs=dist_coeffs)
    ...
    def render_sample(self,
        ...
        num_lidar_plots = 2 if len(lidar_data) > 0 else 0
    ...
    def render_sample_data(self,
        ...
        if sensor_modality in ['lidar', 'radar']:
            ...
            _, boxes, _, _ = self.nusc.get_sample_data(ref_sd_token, box_vis_level=box_vis_level,
                                                    use_flat_vehicle_coordinates=use_flat_vehicle_coordinates)
            ...
            data_path, boxes, camera_intrinsic, dist_coeffs = self.nusc.get_sample_data(sample_data_token,
            ...
            if with_anns:
                for box in boxes:
                    c = np.array(self.get_color(box.name)) / 255.0
                    box.render(ax, view=camera_intrinsic, normalize=True, colors=(c, c, c),dist_coeffs=dist_coeffs)
    ...
    def render_annotation(self,
        ...
        for cam in cams:
            _, boxes, _,_ = self.nusc.get_sample_data(sample_record['data'][cam], box_vis_level=box_vis_level,
        ...
        data_path, boxes, camera_intrinsic,_ = self.nusc.get_sample_data(lidar, selected_anntokens=[anntoken])
        ...
        # Plot CAMERA view.
        data_path, boxes, camera_intrinsic,dist_coeffs = self.nusc.get_sample_data(cam, selected_anntokens=[anntoken])
        ...
        for box in boxes:
            c = np.array(self.get_color(box.name)) / 255.0
            box.render(axes[1], view=camera_intrinsic, normalize=True, colors=(c, c, c),dist_coeffs=dist_coeffs)
        ...
        while current_time < last_sample_rec['timestamp']:
            ...
            for channel, sd_rec in current_recs.items():
                ...
                if not sd_rec == prev_recs[channel]:
                    ...
                    impath, boxes, camera_intrinsic,dist_coeffs = self.nusc.get_sample_data(sd_rec['token'],
                    ...
                    for box in boxes:
                        ...
                        box.render_cv2(im, view=camera_intrinsic, normalize=True, colors=(c, c, c),dist_coeffs=dist_coeffs)
    ...
    def _plot_points_and_bboxes(self,
        ...
        if with_anns:
            # Get annotations and params from DB.
            impath, boxes, camera_intrinsic,dist_coeffs = self.nusc.get_sample_data(camera_token, box_vis_level=BoxVisibility.ANY)
