...
def render_sample_data(
        ...
        pred_data_lidar=None,
        pred_data_camera=None,
    ...
    pred_detection_boxes_lidar = []
    for record in pred_data_lidar['results'][sample_toekn]:
        if record['detection_score'] > det_score_thresh:
            pred_detection_boxes_lidar.append(DetectionBox(
    ...
    boxes_pred_sensor_lidar = boxes_to_sensor(pred_detection_boxes_lidar, pose_record, cs_record)
    boxes_gt_sensor = boxes_to_sensor(gt_detection_boxes, pose_record, cs_record)
    
    # Add scores to predicted boxes
    for box_pred, box_pred_global in zip(boxes_pred_sensor_lidar, pred_detection_boxes_lidar):
    ...
    for box in boxes_pred_sensor_lidar:
    ...
    for ind, cam in enumerate(cams):
        ...
        if sensor_modality in ['lidar', 'radar']:
            ...
            boxes = [Box(record['translation'], record['size'], Quaternion(record['rotation']),
                     ...
                     pred_data_camera['results'][sample_toekn] if record['detection_score'] > det_score_thresh and record['detection_name']!='construction_vehicle']
...
if __name__ == '__main__':
    ...
    parser.add_argument('--result_json_lidar', type=str, required=True, help='Path to detection result JSON file')
    parser.add_argument('--result_json_camera', type=str, required=True, help='Path to detection result JSON file')
    ...
    bevformer_results_lidar = mmcv.load(args.result_json_lidar)
    bevformer_results_camera = mmcv.load(args.result_json_camera)
    sample_token_list = list(bevformer_results_lidar['results'].keys())
    ...
    for idx in range(0, min(args.max_samples, len(sample_token_list)), args.step):
        ...
        render_sample_data(token, pred_data_lidar=bevformer_results_lidar, pred_data_camera=bevformer_results_camera, out_path=out_path,
