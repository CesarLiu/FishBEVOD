...
def render_sample_data(
    ...
    gs_pred = gridspec.GridSpec(3, 2, height_ratios=[3.5, 1, 3.5], width_ratios=[1, 1], 
    ...
    ax_pred = [fig_pred.add_subplot(gs_pred[1, 0]), fig_pred.add_subplot(gs_pred[1, 1]),
               fig_pred.add_subplot(gs_pred[2, 0]), fig_pred.add_subplot(gs_pred[2, 1])]
    ...
    for ind, cam in enumerate(cams):
        ...
        ax_pred[ind].set_title('PRED: {}'.format(sd_record['channel']), fontsize=10, pad=5)
