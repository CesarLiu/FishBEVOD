...
#         ]
#     if ax is None:
#         _, ax = plt.subplots(4, 2, figsize=(24, 18))
#     j = 0
...
#             print('Visualize on Camera with orignal prediction number:', len(pred_data['results'][sample_toekn]))
...
#                     c = np.array(get_color(box.name)) / 255.0
#                     box.render(ax[j, ind], view=camera_intrinsic, normalize=True, colors=(c, c, c),dist_coeffs=dist_coeffs)
#                 for box in boxes_gt:
#                     c = np.array(get_color(box.name)) / 255.0
#                     box.render(ax[j + 2, ind], view=camera_intrinsic, normalize=True, colors=(c, c, c),dist_coeffs=dist_coeffs)
...
def render_sample_data(
    ...
    if ax is None:
        # Create custom layout with gridspec
        fig = plt.figure(figsize=(24, 24))
        
        # Create grid: 
        # - Top 2 rows for front cameras (wide aspect ratio): height_ratios [1, 1]
        # - Bottom 2 rows for rear cameras (square aspect ratio): height_ratios [2, 2]
        gs = gridspec.GridSpec(4, 2, 
                              height_ratios=[1, 2, 1, 2],  # Smaller height for front cams, larger for rear
                              width_ratios=[1, 1],
                              hspace=0.15, 
                              wspace=0.05)
        
        # Create subplot array manually
        ax = []
        for i in range(4):
            row = []
            for j in range(2):
    ...
    for ind, cam in enumerate(cams):
        ...
        if sensor_modality in ['lidar', 'radar']:
            ...
            # Show image.
            ax[j][col].imshow(data)
            ax[j + 2][col].imshow(data)

            # Show boxes.
            if with_anns:
                for box in boxes_pred:
                    c = np.array(get_color(box.name)) / 255.0
                    box.render(ax[j][col], view=camera_intrinsic, normalize=True, colors=(c, c, c),dist_coeffs=dist_coeffs)
                for box in boxes_gt:
                    c = np.array(get_color(box.name)) / 255.0
                    box.render(ax[j + 2][col], view=camera_intrinsic, normalize=True, colors=(c, c, c),dist_coeffs=dist_coeffs)

            # Limit visible range.
            ax[j][col].set_xlim(0, data.size[0])
            ax[j][col].set_ylim(data.size[1], 0)
            ax[j + 2][col].set_xlim(0, data.size[0])
            ax[j + 2][col].set_ylim(data.size[1], 0)

        else:
            raise ValueError("Error: Unknown sensor modality!")

        ax[j][col].axis('off')
        ax[j][col].set_title('PRED: {} {labels_type}'.format(
            sd_record['channel'], labels_type='(predictions)' if lidarseg_preds_bin_path else ''))
        ax[j][col].set_aspect('equal')

        ax[j + 2][col].axis('off')
        ax[j + 2][col].set_title('GT:{} {labels_type}'.format(
            sd_record['channel'], labels_type='(predictions)' if lidarseg_preds_bin_path else ''))
        ax[j + 2][col].set_aspect('equal')
...
if __name__ == '__main__':
    ...
    for idx in range(0, min(args.max_samples, len(sample_token_list)), args.step):
        out_path = os.path.join(args.output_dir, sample_token_list[idx])
        render_sample_data(sample_token_list[idx], pred_data=bevformer_results, out_path=out_path,with_distortion=args.with_distortion,det_score_thresh=args.det_score_thresh)
