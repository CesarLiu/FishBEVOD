...
data = dict(
    ...
    train=dict(
        type=dataset_type,
        data_root=data_root,
        ann_file=data_root + 'mini_bevformer_infos_temporal_train.pkl',
    ...
    val=dict(type=dataset_type,
             data_root=data_root,
             ann_file=data_root + 'mini_bevformer_infos_temporal_val.pkl',
...
optimizer = dict(
    ...
    paramwise_cfg=dict(
        custom_keys={
            'img_backbone': dict(lr_mult=0.0, decay_mult=0.0),   # freeze
            'neck':         dict(lr_mult=0.0, decay_mult=0.0),   # freeze
            'bbox_head':    dict(lr_mult=1.0, decay_mult=1.0), # train
        }),
    weight_decay=0.01)
...
load_from = 'work_dirs/bevformer_small_s_fixed/epoch_12.pth'
log_config = dict(
    interval=50,
    hooks=[
