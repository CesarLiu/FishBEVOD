...
class NuScenesEval_custom(NuScenesEval):
    ...
    def __init__(self,
        ...
        # Debug the sample token differences
        pred_tokens = set(self.pred_boxes.sample_tokens)
        gt_tokens = set(self.gt_boxes.sample_tokens)
        
        print(f"Number of prediction samples: {len(pred_tokens)}")
        print(f"Number of ground truth samples: {len(gt_tokens)}")
        print(f"Number of common samples: {len(pred_tokens.intersection(gt_tokens))}")
        
        print("Samples in predictions but not in ground truth:")
        for token in sorted(list(pred_tokens - gt_tokens)):  # Show first 5
            print(f"  {token}")
        
        print("Samples in ground truth but not in predictions:")
        for token in sorted(list(gt_tokens - pred_tokens))[:5]:  # Show first 5
            print(f"  {token}")
