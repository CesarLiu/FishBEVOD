...
class BEVFormerPolarHead(DETRHead):
    """Head of Detr3D.
    Args:
    ...
    def __init__(self,
        ...
        super(BEVFormerPolarHead, self).__init__(
    ...
    def forward(self, mlvl_feats, img_metas, prev_bev=None,  only_bev=False):
        ...
        for lvl in range(hs.shape[0]):
            ...
            # reference is normalized cylindrical (rho, phi, z)
            # apply offsets in cylindrical space
            tmp[..., 0:2] += reference[..., 0:2]
            tmp[..., 0:2] = tmp[..., 0:2].sigmoid()
            tmp[..., 4:5] += reference[..., 2:3]
            tmp[..., 4:5] = tmp[..., 4:5].sigmoid()

            # Un-normalize cylindrical coordinates and convert to Cartesian
            # pc_range is [xmin, ymin, zmin, xmax, ymax, zmax]
            max_rho = (self.pc_range[3]**2 + self.pc_range[4]**2)**0.5
            
            # Un-normalize rho, phi, z
            rho = tmp[..., 0:1] * max_rho
            phi = (tmp[..., 1:2] - 0.5) * (2 * math.pi) # Map [0, 1] to [-pi, pi]
            z = tmp[..., 4:5] * (self.pc_range[5] - self.pc_range[2]) + self.pc_range[2]

            # Convert to Cartesian x, y
            x = rho * torch.cos(phi)
            y = rho * torch.sin(phi)

            # Update the tmp tensor with un-normalized Cartesian coordinates
            tmp[..., 0:1] = x
            tmp[..., 1:2] = y
            tmp[..., 4:5] = z
...
@HEADS.register_module()
class BEVFormerPolarHead_GroupDETR(BEVFormerPolarHead):
    def __init__(self,
                 *args,
                 group_detr=1,
                 **kwargs):
