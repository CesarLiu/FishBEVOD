...
def render_annotation(
    ...
    data_path, boxes, camera_intrinsic, distortion_coefficients = nusc.get_sample_data(lidar, selected_anntokens=[anntoken])
    ...
    for i in range(1, num_cam + 1):
        cam = select_cams[i - 1]
        data_path, boxes, camera_intrinsic,distortion_coefficients = nusc.get_sample_data(cam, selected_anntokens=[anntoken])
        ...
        for box in boxes:
            c = np.array(get_color(box.name)) / 255.0
            box.render(axes[i], view=camera_intrinsic, normalize=True, colors=(c, c, c), distortion_coefficients=distortion_coefficients)
...
def get_sample_data(sample_data_token: str,
    ...
    if sensor_record['modality'] == 'camera':
        ...
        distortion_coefficients = np.array(cs_record['distortion_coefficients']).flatten() 
    ...
    return data_path, box_list, cam_intrinsic, distortion_coefficients
...
def get_predicted_data(sample_data_token: str,
    ...
    if sensor_record['modality'] == 'camera':
        cam_intrinsic = np.array(cs_record['camera_intrinsic'])
        distortion_coefficients = np.array(cs_record['distortion_coefficients']).flatten()  # 确保为一维
        imsize = (sd_record['width'], sd_record['height'])
    else:
        cam_intrinsic = None
        imsize = None
    ...
    for box in boxes:
        ...
        if sensor_record['modality'] == 'camera' and not \
                box_in_image(box, cam_intrinsic, imsize, vis_level=box_vis_level,  distortion_coefficients=cs_record['distortion_coefficients']):
            continue
        box_list.append(box)
...
def lidiar_render(sample_token, data,out_path=None):
    ...
    for ann in anns:
        content = nusc.get('sample_annotation', ann)
        detection_name = category_to_detection_name(content['category_name'])
        if detection_name not in ['construction_vehicle']:  # 只绘制 car 和 truck
            try:
                bbox_gt_list.append(DetectionBox(
                    sample_token=content['sample_token'],
                    translation=tuple(content['translation']),
    ...
    for content in bbox_anns:
        if content['detection_name'] not in ['construction_vehicle']:  # 只绘制 car 和 truck
            bbox_pred_list.append(DetectionBox(
                sample_token=content['sample_token'],
                translation=tuple(content['translation']),
                size=tuple(content['size']),
...
def render_sample_data(
    ...
    if ax is None:
        ...
        _, ax = plt.subplots(2, 4, figsize=(24, 12))
    #j = 0
    for ind, cam in enumerate(cams):
        ...
        if sensor_modality in ['lidar', 'radar']:
            ...
            boxes = [Box(record['translation'], record['size'], Quaternion(record['rotation']),
                    ...
                    if record['detection_score'] > 0.2 and record['detection_name'] in ['car', 'truck']]


            data_path, boxes_pred, camera_intrinsic, distortion_coefficients = get_predicted_data(sample_data_token,
                                                                         box_vis_level=box_vis_level, pred_anns=boxes)
            _, boxes_gt, _ , distortion_coefficients= nusc.get_sample_data(sample_data_token, box_vis_level=box_vis_level)
            boxes_gt = [box for box in boxes_gt if box.name in ['vehicle.car', 'vehicle.truck']]
            ...
            if with_anns:
                for box in boxes_pred:
                    c = np.array(get_color(box.name)) / 255.0
                    box.render(ax[0, ind], view=camera_intrinsic, normalize=True, colors=(c, c, c), distortion_coefficients= distortion_coefficients)
                for box in boxes_gt:
                    c = np.array(get_color(box.name)) / 255.0
                    box.render(ax[1, ind], view=camera_intrinsic, normalize=True, colors=(c, c, c), distortion_coefficients=distortion_coefficients)
        ...
        ax[0, ind].axis('off')
        ax[0, ind].set_title('PRED: {} {labels_type}'.format(
            sd_record['channel'], labels_type='(predictions)' if lidarseg_preds_bin_path else ''))
        ax[0, ind].set_aspect('equal')

        ax[1, ind].axis('off')
        ax[1, ind].set_title('GT:{} {labels_type}'.format(
            sd_record['channel'], labels_type='(predictions)' if lidarseg_preds_bin_path else ''))
...
if __name__ == '__main__':
    nusc = NuScenes(version='v1.0-trainval', dataroot='/home/jguo/BEVFormer/data/nuscenes', verbose=True)
    # render_annotation('7603b030b42a4b1caa8c443ccc1a7d52')
    bevformer_results = mmcv.load('/home/jguo/PETR/work_dirs/pp-nus/pts_bbox/results_nusc.json')

    
    
    sample_token_list = list(bevformer_results['results'].keys())
    output_dir = "/home/jguo/PETR/tools/result_vis/PETR_best"
    os.makedirs(output_dir, exist_ok=True)
    for id in range(270, 275):
        render_sample_data(
            sample_token_list[id],
            pred_data=bevformer_results,
            out_path=os.path.join(output_dir, sample_token_list[id])
        )
