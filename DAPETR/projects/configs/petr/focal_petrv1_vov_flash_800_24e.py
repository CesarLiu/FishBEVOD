...
model = dict(
    ...
    img_roi_head=dict(
        ...
        loss_bbox2d=dict(type='L1Loss', loss_weight=1.0), #5.0
        loss_iou2d=dict(type='GIoULoss', loss_weight=0.5),# 2.0
        loss_centers2d=dict(type='L1Loss', loss_weight=10.0),
        train_cfg=dict(
        assigner2d=dict(
            type='HungarianAssigner2D',
            cls_cost=dict(type='FocalLossCost', weight=2.),
            reg_cost=dict(type='BBoxL1Cost', weight=1.0, box_format='xywh'),
...
data = dict(
    ...
    train=dict(
        type=dataset_type,
        data_root=data_root,
        ann_file=data_root + 'kitti360_2d_mini_temporal_infos_train.pkl',
        ...
        use_valid_flag=True,
        box_type_3d='LiDAR'),
    val=dict(type=dataset_type, pipeline=test_pipeline, with_2d_labels=True,ann_file=data_root + 'kitti360_2d_mini_temporal_infos_val.pkl', classes=class_names, modality=input_modality),
    test=dict(type=dataset_type, pipeline=test_pipeline, with_2d_labels=True,ann_file=data_root + 'kitti360_2d_mini_temporal_infos_val.pkl', classes=class_names, modality=input_modality),
