...
class PETRDNHeadPolar(AnchorFreeHead):
    ...
    def __init__(self,
        ...
        if class_weight is not None and (self.__class__ is PETRDNHeadPolar):
        ...
        super(PETRDNHeadPolar, self).__init__(num_classes, in_channels, init_cfg = init_cfg)
        ...
        if self.with_distortion:
            ...
            for img_meta in img_metas:
                ...
                for i in range(len(img_meta['intrinsics'])):
                    lidar2img_ = img_meta['intrinsics'][i] @ img_meta['extrinsics'][i].T
                    img2lidar.append(np.linalg.inv(lidar2img_))
    ...
    def prepare_for_dn(self, batch_size, reference_points, img_metas):
        if self.training:
            ...
            if self.bbox_noise_scale > 0:
                ...
                # Convert noisy Cartesian GT center to normalized cylindrical
                # to match the reference points' format.
                rho = torch.sqrt(known_bbox_center[..., 0]**2 + known_bbox_center[..., 1]**2)
                theta = torch.atan2(known_bbox_center[..., 1], known_bbox_center[..., 0])
                z = known_bbox_center[..., 2]
                
                # Normalize
                max_rho = (self.pc_range[3]**2 + self.pc_range[4]**2)**0.5
                rho_norm = rho / max_rho
                theta_norm = (theta + math.pi) / (2 * math.pi)
                z_norm = (z - self.pc_range[2]) / (self.pc_range[5] - self.pc_range[2])
                
                known_bbox_center = torch.stack([rho_norm, theta_norm, z_norm], dim=-1)
    ...
    def _load_from_state_dict(self, state_dict, prefix, local_metadata, strict,
        ...
        if (version is None or version < 2) and self.__class__ is PETRDNHeadPolar:
            convert_dict = {
