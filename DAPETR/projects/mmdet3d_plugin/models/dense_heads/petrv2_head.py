...
class PETRv2Head(AnchorFreeHead):
    ...
    def forward(self, mlvl_feats, img_metas):
        ...
        batch_size, num_cams = x.size(0), 4
        ...
        if self.with_time:
            ...
            time_stamp = time_stamp.view(batch_size, -1, 4) #change to 4 cameras
            
            #mean_time_stamp = (time_stamp[:, 1, :] - time_stamp[:, 0, :]).mean(-1)
            if time_stamp.size(1) > 1:  # 检查是否有足够的帧
                mean_time_stamp = (time_stamp[:, 1, :] - time_stamp[:, 0, :]).mean(-1)
            else:
                mean_time_stamp = time_stamp[:, 0, :].mean(-1)  # 或者直接使用默认逻辑
