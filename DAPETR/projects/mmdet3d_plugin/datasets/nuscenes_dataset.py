...
class CustomNuScenesDataset(NuScenesDataset):
    ...
    def __init__(self, with_2d_labels=False, *args, **kwargs):
        super().__init__(*args, **kwargs)
        self.with_2d_labels = with_2d_labels
        self.org_class_names = ('car', 'truck', 'trailer', 'bus', 'construction_vehicle',
               'bicycle', 'motorcycle', 'pedestrian', 'traffic_cone',
               'barrier','trafficsign')
    ...
    def get_data_info(self, index):
        ...
        if not self.test_mode:
            ...
            if self.with_2d_labels:
                labels2d_raw = info['labels2d']
                labels2d = []
                for lbl in labels2d_raw:
                    lbl = np.array(lbl)
                    lbl[lbl == 4] = -1
                    lbl[lbl > 4] -= 1
                    labels2d.append(lbl)
                ...
                #         lbl_name = self.org_class_names.index(lbl[i]) 
                #         if lbl_name in self.CLASSES:
                #             lbl[i] = self.CLASSES.index(lbl_name)
                #         else:
                #             lbl[i] = -1
                #     labels2d.append(lbl)
                annos.update(
