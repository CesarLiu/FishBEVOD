...
class LoadMixedCameraImageFromFiles(object):
    ...
    def _transform_2d_annotations(self, results, cam_idx, x0, y0, scale_x, scale_y, target_height=376, target_width=1408):
        """Transform 2D annotations (bboxes, centers2d) for fisheye cameras.
        
        Args:
            results (dict): Results dictionary containing annotations
            cam_idx (int): Camera index
            x0, y0 (int): Crop offsets
            scale_x, scale_y (float): Scale factors for resizing
        """
        # Transform bboxes2d if they exist
        if 'bboxes' in results['ann_info'] and cam_idx < len(results['ann_info']['bboxes']):
            bboxes = results['ann_info']['bboxes'][cam_idx]
            if bboxes is not None and len(bboxes) > 0:
                ...
                bboxes[:, [0, 2]] = np.clip(bboxes[:, [0, 2]], 0, target_width)  # width
                bboxes[:, [1, 3]] = np.clip(bboxes[:, [1, 3]], 0, target_height)  # height

                results['ann_info']['bboxes'][cam_idx] = bboxes

        # Transform centers2d if they exist
        if 'centers2d' in results['ann_info'] and cam_idx < len(results['ann_info']['centers2d']):
            centers2d = results['ann_info']['centers2d'][cam_idx]
            if centers2d is not None and len(centers2d) > 0:
                # centers2d format: [cx, cy]
                # Apply cropping
                centers2d[:, 0] -= x0  # x coordinates
                centers2d[:, 1] -= y0  # y coordinates
                
                # Apply scaling
                centers2d[:, 0] *= scale_x  # x coordinates
                centers2d[:, 1] *= scale_y  # y coordinates
                
                # Clamp to image boundaries
                centers2d[:, 0] = np.clip(centers2d[:, 0], 0, target_width)  # width
                centers2d[:, 1] = np.clip(centers2d[:, 1], 0, target_height)  # height

                results['ann_info']['centers2d'][cam_idx] = centers2d
    ...
    def __call__(self, results):
        ...
        for i in range(self.num_fisheye):
            ...
            if self.fisheye_crop_regions:
                ...
                # Get fisheye camera index
                fisheye_cam_idx = self.num_pinhole + i
                
                # Transform 2D annotations for this fisheye camera
                if self.with_2d:
                    self._transform_2d_annotations(results, fisheye_cam_idx, x0, y0, scale_x, scale_y,target_height, target_width)
                ...
                for cam_key in cam_keys:
                    if cam_key in results:
                        # Adjust principal point for cropping
                        results[cam_key][fisheye_cam_idx][0, 2] -= x0  # cx adjustment
                        results[cam_key][fisheye_cam_idx][1, 2] -= y0  # cy adjustment
                        
                        # Scale intrinsics for resize
                        # fx and fy are scaled by scale factors
                        results[cam_key][fisheye_cam_idx][0, 0] *= scale_x  # fx adjustment
                        results[cam_key][fisheye_cam_idx][1, 1] *= scale_y  # fy adjustment
                        # cx and cy are also scaled by scale factors
                        results[cam_key][fisheye_cam_idx][0, 2] *= scale_x  # cx adjustment
                        results[cam_key][fisheye_cam_idx][1, 2] *= scale_y  # cy adjustment
...
@PIPELINES.register_module()
class LoadMultiViewImageFromFilesDrop(object):
    """Load multi channel images from a list of separate channel files.
    ...
    def __init__(self, to_float32=False, color_type='unchanged',
                 drop_strategy='zeros', drop_img_indices=None, noise_std: float = 0.05):
        self.to_float32 = to_float32
        self.color_type = color_type
        # Drop-image options similar to LoadMixedCameraImageFromFiles
        self.drop_img_indices = drop_img_indices or []
        self.drop_strategy = drop_strategy or 'zeros'
    ...
    def _create_mock_image(self, target_shape, strategy='zeros'):
        ...
        Args:
            target_shape (tuple): Target image shape (H, W, C)
            strategy (str): Mock strategy to use ('zeros' | 'noise' | 'mean').
        ...
        if strategy == 'zeros':
            ...
            mock_img = np.full(target_shape, 127, dtype=np.uint8)
        else:
            mock_img = np.zeros(target_shape, dtype=np.uint8)
        return mock_img
    ...
    def __call__(self, results):
        ...
        """
        filename = results['img_filename']
        # Load images individually to support dropping selected views.
        imgs = [mmcv.imread(name, self.color_type) for name in filename]

        if len(imgs) == 0:
            raise ValueError('results["img_filename"] is empty.')

        ref_shape = imgs[0].shape  # assume same shape for this loader

        # Apply drop policy
        for i in self.drop_img_indices:
            if 0 <= i < len(imgs):
                imgs[i] = self._create_mock_image(ref_shape, self.drop_strategy)
        ...
        h, w = ref_shape[:2]
        c = 1 if len(ref_shape) < 3 else ref_shape[2]
        n = len(imgs)
        stacked_shape = (h, w, c, n)
        results['img_shape'] = stacked_shape
        results['ori_shape'] = stacked_shape
        # Set initial values for default meta_keys
        results['pad_shape'] = stacked_shape
        results['scale_factor'] = 1.0
        num_channels = c
        results['img_norm_cfg'] = dict(
            mean=np.zeros(num_channels, dtype=np.float32),
    ...
    def __repr__(self):
        ...
        repr_str += f'(to_float32={self.to_float32}, '
        repr_str += f"color_type='{self.color_type}', "
        repr_str += f"drop_strategy='{self.drop_strategy}', "
        repr_str += f"drop_img_indices={self.drop_img_indices})"
        return repr_str