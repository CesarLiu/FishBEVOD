...
class CustomCollect3D(object):
    ...
    def __init__(self,
                 ...
                 meta_keys=('filename', 'ori_shape', 'img_shape', 'lidar2img','lidar2cam',
                            ...
                            'can_bus','dist_coeffs','intrinsics','extrinsics'
                            )):
        self.keys = keys
...
class GlobalRotScaleTransImage(object):
    ...
    def scale_xyz(self, results, scale_ratio):
        ...
        for view in range(num_view):
            ...
            results["lidar2img"][view] = (torch.as_tensor(results["lidar2img"][view]).float() @ rot_mat_inv).numpy()
            results["extrinsics"][view] = (torch.as_tensor(rot_mat_inv.T @ results["extrinsics"][view]).float()).numpy()
        return
